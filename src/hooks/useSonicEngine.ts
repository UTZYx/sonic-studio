...
export function useSonicEngine(
    ...
    // Ref pattern to keep igniteSegment stable while accessing latest state
    ...
    const igniteSegment = useCallback(async (segmentId: string) => {
        // Use ref to access latest segments without adding to dependency array
        ...
        if (segment.usePreviousContext) {
            ...
            if (index > 0) {
                const prevSegment = currentSegments[index - 1];
    ...
    }, [setTimelineSegments, addLog]); // Stability optimized: no timelineSegments dependency
