...
export function useSonicEngine(
    ...
    // Use ref to access latest segments without re-creating callback
    const segmentsRef = useRef(timelineSegments);
    useEffect(() => { segmentsRef.current = timelineSegments; }, [timelineSegments]);
