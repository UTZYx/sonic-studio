...
export function Timeline({ segments, setSegments, onGenerateSegment, onPlayChain, activeIndex = -1 }: TimelineProps) {
    ...
    const addSegment = () => {
        ...
        // Functional update for consistency, though setSegments prop might be just setter from useState
        ...
        <div className="flex h-full w-full overflow-x-auto p-4 gap-4">
            ...
            {segments.length === 0 && (
                <div className="w-full flex flex-col items-center justify-center text-neutral-700 space-y-2 border-2 border-dashed border-white/5 rounded-2xl">
                    ...
                    {/* Add Segment Button (implied missing from original code but helpful) */}
                    <button
                        onClick={addSegment}
                        className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors mt-2"
                    >
                        + Initialize
                    </button>
                </div>
            ...
            {/* Add Button at end of list if not empty */}
             {segments.length > 0 && (
                <motion.div
                    layout
                    className="flex-none flex items-center justify-center w-12 border-l border-white/5 opacity-50 hover:opacity-100 transition-opacity"
                >
                    <button
                        onClick={addSegment}
                        className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-cyan-500 hover:text-black transition-all"
                        title="Add Segment"
                    >
