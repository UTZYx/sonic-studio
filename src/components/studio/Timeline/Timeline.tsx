...
export function Timeline({ segments, setSegments, onGenerateSegment, onPlayChain, activeIndex = -1 }: TimelineProps) {
    ...
    // Stable Ref for onGenerateSegment to prevent re-renders of children
    const onGenerateRef = useRef(onGenerateSegment);
    useEffect(() => {
        onGenerateRef.current = onGenerateSegment;
    }, [onGenerateSegment]);

    const handleGenerate = useCallback((id: string) => {
        onGenerateRef.current(id);
    }, []);
    ...
    return (
        <div className="flex h-full w-full overflow-x-auto p-4 gap-4">
            ...
            {segments.length === 0 && (
                <div
                    onClick={addSegment}
                    className="w-full flex flex-col items-center justify-center text-neutral-700 space-y-2 border-2 border-dashed border-white/5 rounded-2xl cursor-pointer hover:bg-white/5 transition-colors"
                >
                    <span className="text-xs font-mono uppercase">Timeline Empty</span>
                    <span className="text-[9px]">Click to add {selectedType.label} block</span>
                </div>
            )}
        </div>
    );
