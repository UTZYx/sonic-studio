...
export function Timeline({ segments, setSegments, onGenerateSegment, onPlayChain, activeIndex = -1 }: TimelineProps) {
    ...
    const duplicateSegment = (segment: TimelineSegment) => {
    ...
    const toggleLayers = (segment: TimelineSegment) => {
    ...
    const addLayer = (segmentId: string) => {
        const segment = segments.find(s => s.id === segmentId);
        if (!segment || !segment.layers) return;
        ...
        updateSegment(segmentId, { layers: [...segment.layers, newLayer] });
    };

    const removeLayer = (segmentId: string, layerId: string) => {
        const segment = segments.find(s => s.id === segmentId);
        if (!segment || !segment.layers) return;
                            ...
                            <div className="flex-1 p-2 flex flex-col gap-1 overflow-y-auto relative">
                                {segment.layers.map((layer) => (
                                    <div key={layer.id} className="flex items-center gap-2 bg-black/20 p-1.5 rounded border border-white/5 hover:border-white/10 transition-colors group/layer">
                                        ...
                                        {/* Remove Layer */}
                                        <button
                                            onClick={() => removeLayer(segment.id, layer.id)}
