...
import { Trash2, Zap, Copy, Wand2, Repeat, Layers } from "lucide-react";
...
const TimelineItem = memo(function TimelineItem({
    ...
    const handleLayerUpdate = (layerId: string, updates: Partial<TimelineLayer>) => {
        if (!segment.layers) return;
        const newLayers = segment.layers.map(l => l.id === layerId ? { ...l, ...updates } : l);
        onUpdate(segment.id, { layers: newLayers });
    };
    ...
    const handleToggleLayers = () => {
        if (segment.layers && segment.layers.length > 0) {
            ...
            const defaultLayers: TimelineLayer[] = [
                ...
                { id: uuidv4(), role: "core", provider: "local-gpu", prompt: segment.prompt || "Main rhythm, strong melody...", active: true, volume: 1.0, pan: 0 },
                { id: uuidv4(), role: "detail", provider: "cloud-eleven", prompt: "Ear candy, glitches, foley...", active: true, volume: 0.7, pan: 0.3 },
            ];
            onUpdate(segment.id, { layers: defaultLayers });
        }
    };
        ...
        >
            ...
            <div className="p-3 border-b border-white/5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`p-1 rounded hover:bg-white/10 transition-colors ${segment.layers && segment.layers.length > 0 ? `text-${segment.color}-400` : "text-neutral-600"}`}
                        title="Toggle Field Layers"
                    >
            ...
            <div className="px-3 py-1.5 border-b border-white/5 flex items-center justify-between bg-black/10">
                <input
                    type="text"
                    value={segment.mood}
                    onChange={(e) => onUpdate(segment.id, { mood: e.target.value })}
                ...
                <div className="flex items-center gap-0.5">
                    <button
                        onClick={() => onUpdate(segment.id, { density: "low" })}
            ...
            {segment.layers && segment.layers.length > 0 ? (
                <div className="flex-1 p-2 flex flex-col gap-1 overflow-y-auto relative">
                    {segment.layers.map((layer) => (
                        <div key={layer.id} className="flex items-center gap-2 bg-black/20 p-1.5 rounded border border-white/5 hover:border-white/10 transition-colors group/layer">
                            {/* Active Toggle */}
                            <button
                                onClick={() => handleLayerUpdate(layer.id, { active: !layer.active })}
                            ...
                            <div className="flex flex-col w-12">
                                <input
                                    value={layer.role}
                                    onChange={(e) => handleLayerUpdate(layer.id, { role: e.target.value as any })}
                            ...
                            <input
                                value={layer.prompt}
                                onChange={(e) => handleLayerUpdate(layer.id, { prompt: e.target.value })}
                            ...
                            <div className="flex items-center gap-2 pr-2 border-l border-white/5 pl-2 opacity-50 group-hover/layer:opacity-100 transition-opacity">
                                ...
                                <div className="flex flex-col items-center w-8">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => handleLayerUpdate(layer.id, { volume: parseFloat(e.target.value) })}
                                ...
                                <div className="flex flex-col items-center w-8">
                                    ...
                                    <input
                                        ...
                                        value={layer.pan ?? 0}
                                        onChange={(e) => handleLayerUpdate(layer.id, { pan: parseFloat(e.target.value) })}
                ...
                <div className="flex-1 p-3 flex flex-col gap-2 relative">
                    <textarea
                        ...
                        onChange={(e) => onUpdate(segment.id, { prompt: e.target.value })}
                    ...
                    <div className="absolute right-2 top-2 flex flex-col gap-1">
                        <button
                            onClick={() => onUpdate(segment.id, { enhancePrompt: !segment.enhancePrompt })}
                        ...
                        <button
                            onClick={() => onUpdate(segment.id, { loop: !segment.loop })}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {segment.versions.length > 0 ? (
                            <div className="flex items-center gap-1 bg-white/5 rounded-lg px-2 py-1">
                                {segment.versions.map((v, idx) => (
                                    <button
                                        key={v.id}
                                        onClick={() => onUpdate(segment.id, { selectedVersionId: v.id, audioUrl: v.url })}
                        ...
                        <select
                            ...
                            onChange={(e) => onUpdate(segment.id, { postFx: e.target.value as any })}
                        >
