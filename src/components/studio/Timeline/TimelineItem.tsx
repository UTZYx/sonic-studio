...
interface TimelineItemProps {
    ...
    duplicateSegment: (segment: TimelineSegment) => void;
    onGenerateSegment: (id: string) => void;
}

export const TimelineItem = memo(forwardRef<HTMLDivElement, TimelineItemProps>(function TimelineItem({
    segment,
    updateSegment,
    removeSegment,
    duplicateSegment,
    ...
    const addLayer = () => {
        ...
        updateSegment(segment.id, { layers: [...segment.layers, newLayer] });
    };

    const removeLayer = (layerId: string) => {
        if (!segment.layers) return;
        updateSegment(segment.id, { layers: segment.layers.filter(l => l.id !== layerId) });
    };

    const toggleLayers = () => {
        if (segment.layers && segment.layers.length > 0) {
            updateSegment(segment.id, { layers: [] });
        } else {
            ...
            updateSegment(segment.id, { layers: defaultLayers });
        }
    };
        ...
        >
            ...
            <div className="px-3 py-1.5 border-b border-white/5 flex items-center justify-between bg-black/10">
                <input
                    type="text"
                    value={segment.mood}
                    onChange={(e) => updateSegment(segment.id, { mood: e.target.value })}
                ...
                <div className="flex items-center gap-0.5">
                    <button
                        onClick={() => updateSegment(segment.id, { density: "low" })}
            ...
            {segment.layers && segment.layers.length > 0 ? (
                <div className="flex-1 p-2 flex flex-col gap-1 overflow-y-auto relative">
                    <div className="flex justify-end mb-1">
                         <button
                            ...
                            className="text-[8px] text-neutral-600 hover:text-neutral-400 flex items-center gap-1"
                            title="Switch to Prompt View"
                        >
                ...
                <div className="flex-1 p-3 flex flex-col gap-2 relative">
                    ...
                    <div className="absolute right-2 top-2 flex flex-col gap-1">
                        <button
                            onClick={toggleLayers}
                            className={`p-1 rounded-md transition-all ${segment.layers && segment.layers.length > 0 ? "bg-cyan-500/20 text-cyan-400" : "text-neutral-700 hover:text-neutral-500"}`}
                            title="Field Layers"
                        >
                            <Layers className="w-3 h-3" />
                        </button>
                        <button
                            onClick={() => updateSegment(segment.id, { enhancePrompt: !segment.enhancePrompt })}
                        ...
                        <button
                            onClick={() => updateSegment(segment.id, { loop: !segment.loop })}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {segment.versions.length > 0 ? (
                            <div className="flex items-center gap-1 bg-white/5 rounded-lg px-2 py-1">
                                {segment.versions.map((v, idx) => (
                                    <button
                                        key={v.id}
                                        onClick={() => updateSegment(segment.id, { selectedVersionId: v.id, audioUrl: v.url })}
                        ...
                        <select
                            ...
                            onChange={(e) => updateSegment(segment.id, { postFx: e.target.value as any })}
                        >
